  ...
  if conf.Physical < 0 || conf.Physical == int(s.Physical) {
    if conf.Port < 0 || conf.Port == int(s.Port) {
      ...
      if conf.Advanced > 0 {
        ...
        if ld < ls { ls = ld }
        n := make([]int, ls)
        for i := range(n) { samples[i].SetValue(uint(s.Data[i])) }
      }
    }
  }
}
...
func main() {
  ...
  if conf.Address == 0 { log.Fatalln("No address defined.") }
  if conf.Channels == 0 { log.Fatalln("No channels defined.") }
  if conf.Preview == 0 { conf.Preview = 16 }
  if conf.BarWidth == 0 { conf.BarWidth = 32 }
  tn := conf.Preview
  if conf.Advanced > 0 {
  ...
  app.Run(func(dt time.Duration) {
    ...
    if err != nil { log.Println(err) } else { parse(an) }
    // log.Printf("%f / %f\n", s.Player.CurrentTime(), s.Player.TotalTime())
    if an.Physical == uint8(conf.Physical) && an.Port == uint16(conf.Port) { app.UI.Data = an.Data[0:tn] }
  })
}
